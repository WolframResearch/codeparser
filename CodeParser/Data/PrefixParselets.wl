...
Token`Error`UnhandledDot -> Parselet`PrefixErrorParselet[],
Token`Error`UnhandledCharacter -> Parselet`PrefixErrorParselet[],
Token`Error`ExpectedLetterlike -> Parselet`PrefixErrorParselet[],
Token`Error`ExpectedAccuracy -> Parselet`PrefixErrorParselet[],
Token`Error`ExpectedExponent -> Parselet`PrefixErrorParselet[],
Token`Error`Aborted -> Parselet`PrefixErrorParselet[],
Token`Error`ExpectedOperand -> Parselet`PrefixErrorParselet[],
Token`Error`UnrecognizedDigit -> Parselet`PrefixErrorParselet[],
Token`Error`ExpectedDigit -> Parselet`PrefixErrorParselet[],
Token`Error`UnsupportedCharacter -> Parselet`PrefixErrorParselet[],
Token`Error`InvalidBase -> Parselet`PrefixErrorParselet[],
